...
def main():
	cities = []
	with open('cities.json', 'r') as ofile:
	...
	for city in cities:
		...
		try:
			download_census_data(city, year)
...
def download_census_shp(city, year=2017):
	base_url = "https://www2.census.gov/geo/tiger/TIGER%s/TRACT/tl_%s_--FIPS--_tract.zip"%(year, year)
...
def download_census_data(city, year):
	state = city['state']
	county = city['county']
	...
	# use cenpy api key
	api_key = 'a4b2eab7c7050050923fffa485fb81e22be63e68'
	...
	query_variables = ["NAME","GEO_ID", "B02001_002E", \
										"B01001H_001E", \
										"B02001_003E", \
										"B02001_005E", \
										"B03002_012E", \
	...
	query_keys = {"B02001_002E": "white population", \
					"B01001H_001E": "non-hispanic white population", \
					...
					"B25034_001E": "built age",\
					"B06009_002E": "less highschool", \
					"B06009_005E": "bachelor", \
					"B06009_001E": "totalEducation", \
					"B05003_003E": "male_und18", \
					"B05003_014E": "female_und18", \
					"B01001_020E": "male65_66", \
					"B01001_021E": "male67_69", \
					"B01001_022E": "male70_74", \
					"B01001_023E": "male75_79", \
					"B01001_024E": "male80_84", \
					"B01001_025E": "male85_", \
					"B01001_044E": "fema_65_66", \
					"B01001_045E": "fema_67_69", \
					"B01001_046E": "fema70_74", \
					"B01001_047E": "fema75_79", \
					"B01001_048E": "fema80_84", \
					"B01001_049E": "fema85_"
					}
	...
	# print('The url request is:', request_url)
...
def get_fips(statename):
	...
	with open("state_fips_full.json", "r") as ofile:
...
def merge_census_shp(city, year):
	...
	for file in glob.glob("data/output/" + clean_path_name(city['state']) + "/*_%s*_tract.shp"%(year)):
		state_shp_path = file
